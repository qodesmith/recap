/**
 * PROTOTYPE — the simulated world behind Recap's recording modal (issue #25).
 *
 * Lifted almost verbatim from the #24 prototype, which the ticket says is
 * reusable as-is. Only additions: `discard()`, and the `discarded` flag the
 * README's "was Discard reachable by accident?" question is judged against.
 *
...
export type World = {
  ...
  /** Last discard, so the prototype can show what it cost. */
  discarded: {at: number; n: number}
...
export function discard(w: World) {
  if (w.phase !== 'capturing') return
  w.recordings = w.recordings.filter(r => r.id !== w.capturingId)
  w.discarded = {at: w.elapsed, n: w.discarded.n + 1}
  w.phase = 'setup'
  w.elapsed = 0
  w.capturingId = null
  w.notices = []
