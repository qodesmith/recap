/**
 * VARIANT A — "Conversation"
 *
 * The transcript as a chat log: one bubble per Segment, avatar + colour per
 * Speaker, a Speakers rail on the right, and a player bar pinned to the bottom.
 *
 * Its answers to #14:
 *  - Layout: speaker-grouped bubbles, chat-like.
 *  - Sync: word-level highlight, auto-follow with a "Jump to playhead" pill the
 *    moment you scroll away.
 *  - Seek: click a word, or scrub the waveform in the player bar.
 *  - Edit: double-click a bubble → inline textarea. ⌘↵ / blur commits, Esc cancels.
 *  - Speakers: right rail. Rename inline; merge via each speaker's ⋯ menu.
 *  - Reassign: drag the bubble onto a speaker in the rail (or its chip menu).
 *  - Overlap: overlapping bubbles are tied together with an "at the same time" rail.
 *  - Export: header menu; hover-checkboxes make a selection exportable.
 */
...
export function VariantA({data, overlaps}: VariantProps) {
  ...
  const virt = useVirtualizer({
    ...
    estimateSize: i => 78 + Math.min(520, data.segments[i].words.length * 1.9),
    overscan: 6,
        ...
        <div className="relative ml-auto">
          ...
          {exportOpen && (
            <div
              ...
              className="absolute right-0 z-30 mt-1 w-56 rounded-lg border border-white/10 bg-ink-2 p-1 text-xs shadow-2xl">
              {[
                selected.length ? `Copy selection (${selected.length})` : 'Copy all to clipboard',
      ...
      <div className="flex min-h-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div
                        ...
                        <div className="absolute -top-2 right-2 hidden items-center gap-1 group-hover/row:flex">
                          <button
                            onClick={() => store.toggleSelect(seg.id, true)}
        ...
        <aside className="w-64 shrink-0 border-l border-white/8 px-4 py-3">
              ...
              return (
                <div
                  ...
                  <div className="mt-1 pl-4.5 text-[10px] text-slate-500">
                    {t.n} segments · {fmt(t.secs)} · {sp.provenance.attributed ? 'attributed' : `diarized ${sp.provenance.diarizationLabel}`}
                  </div>
          ...
          <div className="mt-4 text-[10px] leading-relaxed text-slate-500">
            Drag a bubble onto a speaker to reassign it. Diarization over-split this
            recording — “Speaker 3” is really Speaker 2.
          </div>
        </aside>
      </div>
...
function EditBox({
  ...
  return (
    <textarea
      ...
      rows={Math.min(14, Math.ceil(v.length / 90) + 1)}
...
function PlayerBar({data}: {data: VariantProps['data']}) {
